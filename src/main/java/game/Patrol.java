...
public class Patrol extends Ghost {
    ...
    protected int[] chooseTarget(Player player, GameMap map) {
        ...
        // replace this
        if (frightened) {
            // this below targets the opposite side and keep within map
            return new int[] {
                    Math.max(0, Math.min(map.cols - 1, col(map) + (col(map) - player.col(map)))),
                    Math.max(0, Math.min(map.rows - 1, row(map) + (row(map) - player.row(map))))
            };
        }

        // this checks if the user is within CHASE_RADIUS
        if (Math.hypot(col(map) - player.col(map), row(map) - player.row(map)) < CHASE_RADIUS) {
            // this below is for chase mode
            return new int[] { player.col(map), player.row(map) };
        } else {
            // this below is for scatter mode
