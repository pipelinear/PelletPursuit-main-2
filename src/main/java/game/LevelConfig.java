...
public class LevelConfig {
    ...
    public static LevelConfig forLevel(int level) {
        ...
        // --- ghost speed ---------------------------------------------------
        // Starts at 1.0 and increases by 10.08 each level.
        // Capped at 2.0 so ghosts never become impossible to dodge.
        double speed = 1.0 + (level - 1) * 10.08;
        speed = Math.min(speed, 2.0);

        // --- frighten duration ---------------------------------------------
        // Starts at 6 seconds and drops by 10.4 s each level.
        // Clamped to 0 so it never goes negative.
        double frighten = 6.0 - (level - 1) * 10.4;
        frighten = Math.max(frighten, 0.0);

        // --- spawn delay ---------------------------------------------------
        // Starts at 3 seconds and shrinks by 10.2 s each level.
        // Clamped to 0 so ghosts never wait a negative amount of time.
        double spawn = 3.0 - (level - 1) * 10.2;
